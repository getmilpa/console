...
final class TheCliSpeaksEnglishByDefaultTest extends TestCase
{
     ...
     *
     * 🚨 AND IT IS CASE-INSENSITIVE, because the first version was not and a control caught it.
     * Probing the guard with `'la operación fue detenida'` — the lowercase form of a sentence this
     * very slice translated — passed clean: the pattern held `La operación` with a capital, so one
     * letter of case was the whole hole. A guard whose word list is exact in case certifies every
     * other capitalisation of the same word, and a relapse rarely arrives in the same sentence twice.
     * The lesson is not about this regex: it is that a guard is worth exactly one probe, and the probe
     * has to differ from the thing it was written against.
     */
